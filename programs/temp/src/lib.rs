...
declare_id!("GuN8f7tuKKaDQ3RjP91hSKEE1Xg6ADgYxecfEu8akt74");
...
pub struct Initialize2<'info> {
    #[account(init, payer = user, space = 10000 ,seeds = [b"treasury",user.key().as_ref()],bump)]
...
pub struct DepositLamports2<'info> {
    #[account(mut, 
        seeds = [b"treasury",user.key().as_ref()], bump
    )]
    pub treasury: Account<'info, Treasury2>,
